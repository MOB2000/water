...
import 'package:water/screens/history/history_manager.dart';
import 'package:water/screens/history/widgets/history_lists.dart';
...
class _HistoryPageState extends State<HistoryPage>
  ...
  List<DrinkHistoryEntry> _currentEntries(
    ...
    if (index == 0) {
      currentEntries = entries
          .where((entry) => HistoryManager.manager
              .isToday(DateTime.fromMillisecondsSinceEpoch(entry.date ?? 0)))
          .toList();
    } else if (index == 1) {
    ...
    currentEntries.sort((a, b) => b.date?.compareTo(a.date ?? 0) ?? 0);
  ...
  List<Widget> _buildStats(List<DrinkHistoryEntry> entries) {
    ...
    double summary = currentEntries.fold(0.0, (t, e) => t + (e.amount ?? 0));
    ...
    if (_currentIndex == 0) {
      ...
      if (currentEntries.isNotEmpty) {
        currentEntries.sort((a, b) => b.date?.compareTo(a.date ?? 0) ?? 0);
        var firstDay = DateTime.fromMillisecondsSinceEpoch(
            currentEntries.firstOrNull?.date ?? 0);

        var lastDay = DateTime.fromMillisecondsSinceEpoch(
            currentEntries.lastOrNull?.date ?? 0);
...
class DrinkHistoryListItem extends StatelessWidget {
  final int amount;
  final DateTime date;

  const DrinkHistoryListItem(this.amount, this.date, {super.key});
